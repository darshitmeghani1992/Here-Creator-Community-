...
7. **No type or lint errors** in `mobile/` (`npm run typecheck`, `expo lint`).
8. **Keep the web build green** — the Next.js app must not break; `mobile/` is
...
17. **Small, reviewable commits** to PR #6 with a clear message; open follow-up
    PRs per the repo's PR conventions.
