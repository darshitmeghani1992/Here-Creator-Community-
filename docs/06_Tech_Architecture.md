...
## Stack
- **Client:** Expo (React Native) + **expo-router** (file-based nav) — `mobile/`
...
The web app at the repo root (Next.js) is a **separate product** sharing the same
Supabase project; `mobile/` is excluded from its Vercel build via `.vercelignore`.
