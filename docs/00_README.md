...
- **IDs & paths** reference the codebase: web app at repo root (Next.js, not
  changed by this product), native app under `mobile/` (Expo).
...
Storage). See `06_Tech_Architecture.md`. The web app in the repo root is a
separate product sharing the same Supabase project.
