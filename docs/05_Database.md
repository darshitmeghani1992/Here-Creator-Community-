...
Postgres on Supabase. The Wall schema is added by
`supabase/migrations/0010_walls.sql` (idempotent) to the **same project** the web
app uses; it only adds new tables and never touches the existing
creators/rooms/messages tables.
...
## Storage
Public bucket **`attachments`** (shared with web app) holds avatars, mark photos,
