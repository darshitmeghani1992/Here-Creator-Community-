...
5. Commit to PR #6; update `10_Analytics` + `14_Changelog`.
...
| Backend (schema, RLS, triggers, realtime) — `0010_walls.sql` | ✅ done |
...
Files: `mobile/app/create.tsx` (wire tiles → `write/[type]`),
`mobile/app/write/[type].tsx` (new Writer), `mobile/src/lib/marks.ts`
